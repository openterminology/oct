      ...
      >
        <g transform="translate(${margin.left + panOffset.x}, ${Math.abs(minX) + margin.top + panOffset.y}) scale(${zoomLevel})">
          ...
          ${nodes.map((n, i) => {
            ...
            const ellipseRx = hasCode ? Math.max(30, codeText.length * 4) : 12;
            const ellipseRy = hasCode ? 12 : 8;
            return html`
              <g 
                key=${i}
                class="node ${cls}" 
              ...
              >
                <ellipse rx="${ellipseRx}" ry="${ellipseRy}"></ellipse>
                ${hasCode && html`
                  <text 
                    dy="4" 
                    text-anchor="middle" 
                ...
                `}
                <text dx="${ellipseRx + 6}" dy="4">${n.data.name}</text>
              </g>
            `;
          })}
        </g>

...
function Tree({ data, width = 960, height = 600 }) {
  ...
  function handleToggle(d, e) {
    // Right click or ctrl+click to show popup instead of toggling
      ...
      >
        <g transform="translate(${margin.left}, ${Math.abs(minX) + margin.top}) scale(${zoomLevel})">
          ...
          ${nodes.map((n, i) => {
            ...
            const isSelected = selectedNode && selectedNode.name === n.data.name && selectedNode.code === n.data.code;
            return html`
              <g 
                key=${i}
                class="node ${cls}" 
              ...
              >
                <circle r="8" fill=${isSelected ? '#ff6b35' : undefined}></circle>
      ...
      ${selectedNode && html`
        ...
        >
          <div style=${{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}>
            <h3 style=${{ margin: 0, color: '#0b1220' }}>${selectedNode.name}</h3>
            <button 
          ...
          ${selectedNode.code && html`
            <div style=${{ marginBottom: '12px' }}>
              <div style=${{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '4px' }}>SNOMED CT Code</div>
              <div style=${{ 
                ...
                color: '#0b1220',
                fontWeight: '600'
              }}>${selectedNode.code}</div>
            </div>
          `}
          <div style=${{ marginBottom: '12px' }}>
            <div style=${{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '4px' }}>System</div>
            <div style=${{ fontSize: '0.9rem', color: '#24303a' }}>http://snomed.info/sct</div>
          </div>
          <button
            onClick=${() => setSelectedNode(null)}
            style=${{
